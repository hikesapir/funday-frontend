...
export default {
  ...
  computed: {
    statusChart() {
      const statusMapCount = this.$store.getters.boardData?.statusMapCount;
      const statuses = this.$store.getters.board?.labels.status;

      if (!statuses || !statusMapCount) return;
      var data = [];
      for (var key in statusMapCount) {
        const status = statuses.find((status) => status.id === key);
        if (status.txt) {
          const statusData = {
    ...
    statuses() {
      const statuses = JSON.parse(
        JSON.stringify(this.$store.getters.board?.labels.status)
      );
